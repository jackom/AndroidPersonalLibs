...
/**
 * @author : zhengminxin
 * @date : 1/22/2021 11:35 AM
 * @desc : 状态栏、导航栏相关  EXPAND_STATUS_BAR权限暂时不申请
 * {@link "https://github.com/Blankj/AndroidUtilCode/blob/master/lib/utilcode/src/main/java/com/blankj/utilcode/util/BarUtils.java"}
 */
public final class BarUtils {
    ...
    private static void addMarginTopEqualStatusBarHeight(Context context, final Window window) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
    ...
    private static void subtractMarginTopEqualStatusBarHeight(Context context, final Window window) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        View withTag = window.getDecorView().findViewWithTag(TAG_OFFSET);
        if (withTag == null) return;
...
//    }
    ...
    /**
     * 隐藏虚拟导航栏。设置导航栏状态监听，避免点击一下后导航栏又出现了
     * @param activity
     */
